...
const EMPTY_FORM = { nombre: "", direccion: "", activa: true };
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        <p className="text-sm text-slate-500">{sucursales.length} sucursal{sucursales.length !== 1 ? "es" : ""}</p>
        <button
          onClick={openCreate}
      ...
      {error && !mode && !deleteTarget && (
        <div className="mb-4 rounded-lg border border-rose-900/40 bg-rose-900/20 px-4 py-3 text-sm text-rose-400">{error}</div>
      )}
      ...
      <div className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-900/40">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-800/50">
            {sucursales.length === 0 && (
              <tr><td colSpan={5} className="py-10 text-center text-slate-600">Sin sucursales creadas</td></tr>
            )}
            {sucursales.map((s) => (
              <tr key={s.id} className="group transition-colors hover:bg-slate-800/30">
                ...
                <td className="px-4 py-3 text-slate-500 text-xs">{s.direccion ?? <span className="text-slate-700">—</span>}</td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={() => openEdit(s)}
                      className="rounded-lg p-1.5 text-slate-500 hover:bg-slate-700 hover:text-emerald-400 transition-colors"
                      title="Editar"
                    >
                      <Pencil size={14} />
                    </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-widest text-slate-500">Dirección</label>
                <input
                  type="text"
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`relative h-6 w-11 rounded-full transition-colors ${form.activa ? "bg-emerald-600" : "bg-slate-700"}`}
                >
                  <span className={`absolute top-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform ${form.activa ? "translate-x-5" : "translate-x-0.5"}`} />
                </button>
                <span className="text-sm font-medium text-slate-300">{form.activa ? "Activa" : "Inactiva"}</span>
              </div>

              {error && (
                <p className="rounded-lg border border-rose-900/40 bg-rose-900/20 px-3 py-2 text-xs text-rose-400">{error}</p>
              )}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-emerald-600 py-2.5 text-sm font-bold text-white hover:bg-emerald-500 disabled:opacity-50 transition-colors"
                >
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
            <h2 className="text-base font-black uppercase text-white mb-2">¿Eliminar sucursal?</h2>
